...
def find_urls(s):
    ...
    finlist = []
    #filter out the urls we don't want
    for i in lst:
        if "." not in i:
            continue
        temp = 0
...
def grab_headlines():
    ...
    opinion = []
    #Use beautiful soup to parse for the right class and add to our list
    for x in BeautifulSoup(html, "html.parser").find('div', attrs = {'class': 'view view-most-read view-id-most_read view-display-id-panel_pane_1 view-dom-id-99658157999dd0ac5aa62c2b284dd266'}).find_all('a'):
        opinion.append(x.string)
    return opinion
...
def get_umsi_data():
    umsi_titles = {}
    #go through all 13 pages
    for x in range(13):
        if x == 0:
            url="https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All"
        else:
            url="https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All&page="+'&page='+str(x)
        #get names and titles and put into dictionary
        names = BeautifulSoup(requests.get(url, headers={'User-Agent': 'SI_CLASS'}).text, 'html.parser').find_all('div', {'class': 'field-item even', 'property': 'dc:title'}) #parsing for all names in directory
        titles = BeautifulSoup(requests.get(url, headers={'User-Agent': 'SI_CLASS'}).text, 'html.parser').find_all('div', {'class': 'field field-name-field-person-titles field-type-text field-label-hidden'}) #parsing for all titles in directory
